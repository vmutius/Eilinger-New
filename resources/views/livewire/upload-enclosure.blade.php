<form class="space-y-6" x-on:submit.prevent="submit" x-data="{
    uploader: null,
    progress: 0,
    ...
    submit() {
        ...
        this.uploader = createUpload({
            file: file,
            endpoint: '{{ route('livewire.upload-enclosure', app()->getLocale()) }}',
            method: 'post',
            headers: {
    ...
    <template x-if="uploader">
        <div class="space-y-1">
            <div class="bg-gray-100 shadow-inner h-3 rounded overflow-hidden">
                <div class="bg-blue-500 h-full transition-all duration-200" x-bind:style="{ width: `${progress}%` }">
                </div>
            </div>
            ...
            <div class="flex items-center space-x-3 text-sm">
                <button type="button" class="text-blue-500"
                    ...
                    x-text="!uploader.paused ? `Pause` : `Resume`">Pause</button>
